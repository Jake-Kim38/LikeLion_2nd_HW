...
const updateTodo = (targetTodo) => {
  // 찾고자 하는 todo와 기존 todo의 id를 비교하여 filter, 없다면 빈칸을 반환하므로 length는 0이 될 것임.
  const exists = todos.filter((todo) => todo.id === targetTodo.id);
  if (exists.length === 0) {
    return console.log(
      `id = (${targetTodo.id})에 맞는 todo를 찾을 수 없습니다.`
    );
  }
  // 찾고자 하는 todo와 기존 todo의 id가 같다면 기존 todo를 targetTodo로 업데이트, 아니면 그대로.
  todos = todos.map((todo) => (todo.id === targetTodo.id ? targetTodo : todo));
};
...
const deleteTodo = (id) => {
  // 찾고자 하는 todo와 기존 todo의 id를 비교하여 filter, 없다면 빈칸을 반환하므로 length는 0이 될 것임.
  const exists = todos.filter((todo) => todo.id === id);
  if (exists.length === 0) {
    return console.log(`id = (${id})에 맞는 todo를 찾을 수 없습니다.`);
  }
  // deleteTodo의 인자로 넘어온 id를 제외한 나머지로 todo 리스트를 갱신
